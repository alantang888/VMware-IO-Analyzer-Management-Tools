...
urlpatterns = [
    ...
    url(r"^check_result/(?P<server_name>[^/]*)/(?P<test_vm>[^/]*)/(?P<test_spec>[^/]*)/(?P<start_time_str>\d{4}-\d{2}-\d{2})/(?P<end_time_str>\d{4}-\d{2}-\d{2})/$", display_test_profile_result, name="display_test_profile_result_in_range"),
